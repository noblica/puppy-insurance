...
test.describe('Sign-up form', () => {
  ...
  test('browser tab title is "Sign up | Puppy Insurance" on sign-up page', async ({ page }) => {
    await expect(page).toHaveTitle('Sign up | Puppy Insurance')
  })
  ...
  test('success page title is "Account created | Puppy Insurance"', async ({ page }) => {
    await fillEmail(page, VALID_EMAIL)
    await fillPassword(page, VALID_PASSWORD)
    await fillConfirmPassword(page, VALID_PASSWORD)
    await checkTerms(page)
    ...
    await expect(page).toHaveURL('/success')
    await expect(page).toHaveTitle('Account created | Puppy Insurance')
  })

  test('keyboard navigation: tab through fields and submit with Enter', async ({ page }) => {
