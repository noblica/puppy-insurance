...
The `verify` job in `.github/workflows/ci.yml` runs `pnpm run lint` (oxlint), `pnpm run format:check` (oxfmt --check), `pnpm run typecheck`, and `pnpm run test` (unit tests) as blocking steps before deploy.
...
- **Positive**: CI verify job (lint + format check) completes near-instantly within the pipeline.
